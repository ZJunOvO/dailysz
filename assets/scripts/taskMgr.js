...
var l = (function () {
    ...
    t.updateTask = function (e, o) {
        ...
        var r = t.baseTask[o];
        var l = t.saveData[o];
        if (r && e.data.state == h.Finish)
            for (var u in (e.temp.get_by == p.ad_reward
                ? (e.data.state = e.data.count >= e.temp.nums ? h.Rewared : h.Receive)
                : (e.data.state = h.Rewared),
            (r[e.temp.key] = e),
            (l[e.temp.key] = e.data),
            _pInfo.default.ins.setTask(o, l),
            _idx.platform.reportEvent(_idx.ERepEvt.dailyTask, {taskId: _lang.lang[e.temp.key]}),
            _evts.default.opE.emit({action: _evts.default.UPDTASK, data: e}),
            e.temp.val))
        ...
        if (i) {
            ...
            for (var c in i)
                if (i.hasOwnProperty(c)) {
                    ...
                    l.temp[u] == e &&
                        ((l.data.count += 1),
                        (l.data.count >= l.temp.nums || l.temp.get_by == p.ad_reward) &&
                            ((l.data.state = h.Finish), (r[c] = l.data)),
                        (s = !0),
                        _evts.default.opE.emit({action: _evts.default.UPDTASK, data: l}));
    ...
    t.getFinishNum = function () {
        for (
            var e = _cfgMgr.default.serverCfg[d.daily],
                o = t.getTaskByType(d.daily),
                n = null == e ? void 0 : e.rules,
                i = 0,
                a = 0,
                s = n.length;
            a < s;
...
exports.taskPro = f;
(function (t) {
    (t.ad_reward = "ad_reward"),
        (t.cg_line = "cg_line"),
        (t.daily_challenge = "daily_challenge"),
        (t.qw_line = "qw_line");
